...
宏不是必须入口。它只是让“现场写控件树”更顺手；当你需要把单个控件按某个参数类型传出去时，直接调用 `into_view()` 或 `into_views()` 往往更清楚。

## IntoView 和 IntoViews

`IntoView` / `IntoViews` 这组能力用于把单个控件转换成框架更通用的控件形态。它不只用于方法返回值，也用于控件参数、模块参数、容器子节点等需要类型对齐的地方。

当前公开写法里，单控件转换是 `IntoView`，多控件转换是 `IntoViews`。在封装方法时，单个控件可以返回 `impl View` 或 `impl IntoView`；多个控件可以返回 `impl IntoViews`：
...
比如 `div(...)` 这类容器接收的是子控件列表。只有一个子控件时，可以直接把它转成列表再传进去：
...
如果某个控件或模块参数要的是一个子控件，就标注 `impl IntoView`；如果要的是一组子控件，就标注 `impl IntoViews`：
...
let confirm = dialog(
    ...
    button("确认").class("w-full").into_views(),
);
```

这样写的重点是让参数和返回值表达清楚：这里需要的是普通控件、通用控件对象，还是一组可以继续交给容器的子控件。

宏和转换能力可以搭配使用。`views![...]` 适合现场写多个子节点，`into_view()` / `into_views()` 适合把一个已经存在的控件表达式对齐到某个参数需要的形态：

| 写法 | 用途 |
| --- | --- |
| `fn f() -> impl View` | 封装一个完整控件，并保留继续链式配置的能力。 |
| `fn f() -> impl IntoView` | 封装一个准备作为子控件传递的单控件。 |
| `fn f() -> impl IntoViews` | 封装一组准备交给容器接收的子控件。 |
| `control.into_view()` | 把单个控件转成通用控件对象。 |
| `control.into_views()` | 把单个控件转成只包含它的通用控件列表。 |
| `views![a, b, c]` | 在控件树现场写子控件列表。 |
| `view!(control)` | 在需要宏写法时，把单个控件转成通用控件对象。 |
...
```rust
let save = primary_button("保存", |view_id, _, _| {
