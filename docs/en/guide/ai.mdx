...
In other words, when you describe an interface structure to AI, it can leverage its understanding of React-style UI organization to quickly migrate to Flor's DSL, helping you complete rapid interface building, structure adjustment and style organization. For specific widget encapsulation writing, see [Framework DSL](/en/guide/use/framework-dsl).

This is another manifestation of Flor's "want everything, want more, want even more" philosophy:

Want the clear structure of widget-based development from traditional GUI frameworks,
Want the expression efficiency of modern frontend declarative UI,
Want to尽可能 utilize the functional UI thinking AI is already familiar with, making layout design easier to generate, modify and iterate.
...
Therefore, Flor's documentation site also chose a documentation framework with AI support capabilities to build. Besides regular documentation for developers to read, this site also provides documentation entries for large language models, convenient for you to provide more model-readable materials when using AI-assisted development.
...
[llms.txt](https://llmstxt.org/) is a standard specification helping LLMs discover and use project documentation. Rspress follows this specification and publishes the following two files:

- <a href="/llms.txt">llms.txt</a>: Structured index file, containing titles, links and brief descriptions of all documentation pages.

```
/llms.txt
```

- <a href="/llms-full.txt">llms-full.txt</a>: Complete content file, merging all documentation page content into a single file.
