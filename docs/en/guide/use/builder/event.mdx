...
After importing `EventBuilder`, all values implementing `ViewIdentity` can chain bind events. Normal widgets, `ViewBox`, and values returned as `impl IntoView` all satisfy this condition.
...
The full-argument form is still available. Reduced parameters are useful when business logic does not care about the widget ID, mouse position, or button state.
