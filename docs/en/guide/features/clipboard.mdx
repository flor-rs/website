...
`clipboard` feature is used to enable system clipboard read/write capability. It适合 implement copy, paste, import/export temporary text等功能.
...
`Clipboard(None)` means not binding specific window. When need to associate clipboard operation with某 window, can pass `Some(window_id)`.
...
`ClipboardApi` provides two类 entries:
