...
Tray capability is exposed through `FlorGui`. `FlorGui.init()?` will initialize tray所需的 internal window.
...
| Writing | Purpose |
| --- | --- |
| `IconSource::from_path(path)` | Load icon file from disk. Windows currently适合传 `.ico`. |
