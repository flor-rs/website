...
The current design blends traditional widget-based development with various design elements from modern web development. Its performance completely matches my vision of a Rust GUI framework that wants everything, wants more, and wants even more.
...
- Terminal application experience is simple, complexity absorbed by framework/widget authors.
...
[2]: `Retained mode` is implemented based on `immediate mode`. For widgets needing high refresh, `retained mode` still has `immediate mode` performance.  
[3]: Provides `atomic class names` during construction. `Atomic class names` are parsed separately by the framework's layout system and corresponding widgets in order, efficiently and controllably building application layout.  
